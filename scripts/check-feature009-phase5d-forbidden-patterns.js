#!/usr/bin/env node
/**
 * check-feature009-phase5d-forbidden-patterns.js
 *
 * Feature 009 Phase 5D: Static Guard / CI Regression
 *
 * Phase 5D extends Phase 5C with deployment-gate partial-failure / orphaned-
 * token recovery modeling and observation-mode high-concurrency validation —
 * pure logic / contract-style code only. This script statically verifies
 * that the new Phase 5D files:
 *
 ...
 *  6. Contain no hardcoded production-bypass / observation-mode-bypass /
...
const PHASE5D_FILES = [
  'catering-system/src/services/realModelConfigApplyDeploymentGateRecoveryService.ts',
  'catering-system/src/services/concurrencyManager.ts',
  'catering-system/src/services/securityAudit.ts',
];
...
for (const rel of PHASE5D_FILES) {
  const filePath = path.join(root, rel);
  if (!fs.existsSync(filePath)) {
    fail(rel, 'expected Phase 5D file does not exist');
    continue;
  }
  ...
  for (const pattern of FORBIDDEN_IMPORT_PATTERNS) {
    if (pattern.test(content)) {
      fail(rel, `forbidden Firestore/firebase-admin import matching ${pattern} — Phase 5D files are pure logic / contract only`);
    }
  }

  for (const pattern of FORBIDDEN_RUNTIME_PATTERNS) {
    if (pattern.test(content)) {
      fail(rel, `forbidden runtime call matching ${pattern} — no real writes/reads/crypto-SDK calls permitted in Phase 5D`);
    }
  }

  for (const pattern of UI_NETLIFY_CLOUD_PATTERNS) {
    if (pattern.test(content)) {
      fail(rel, `forbidden UI/Netlify/Cloud-Function pattern matching ${pattern} — Phase 5D forbids UI, Netlify Functions, and Cloud Functions`);
    }
  }

  for (const pattern of ROLLBACK_CLEANUP_PATTERNS) {
    if (pattern.test(content)) {
      fail(rel, `forbidden rollback/cleanup implementation matching ${pattern} — Phase 5D forbids rollback and cleanup jobs`);
    }
  }
  ...
  for (const pattern of HARDCODED_BYPASS_PATTERNS) {
    if (pattern.test(codeOnly)) {
      fail(rel, `hardcoded bypass pattern matching ${pattern} — production/observation-mode/emergency-disable must remain disabled-by-default with no literal bypass`);
    }
  }

  const kindMatches = [...content.matchAll(/_kind:\s*'(f009_phase5d_[a-z0-9_]+)'/g)].map((m) => m[1]);
  const contractLikeKinds = kindMatches.filter((k) =>
    /(contract|payload|result|plan|state|lifecycle|set|ledger|attempt|input|candidate)$/.test(k));
...
if (violations === 0) {
  console.log(`[OK] Feature 009 Phase 5D static guard: 0 violations across ${PHASE5D_FILES.length} files.`);
  process.exit(0);
} else {
  console.error(`[FAIL] Feature 009 Phase 5D static guard: ${violations} violation(s) found.`);
  process.exit(1);
}
