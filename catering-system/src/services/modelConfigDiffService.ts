import type { TenantId } from '../types/aiBoundary';
import type {
  ModelWeights,
  ModelConfigDiff,
  ConfigVersion,
  DiffHash,
} from '../types/modelConfigApply';
import { asDiffHash, asConfigVersion } from '../types/modelConfigApply';

// ─── Canonical JSON ───────────────────────────────────────────────────────────

export function canonicalizeObject(value: unknown): unknown {
  if (value === null || typeof value !== 'object') return value;
  if (Array.isArray(value)) return value.map(canonicalizeObject);
  const obj = value as Record<string, unknown>;
  return Object.keys(obj)
    .sort()
    .reduce<Record<string, unknown>>((acc, key) => {
      const v = obj[key];
      if (v !== undefined && typeof v !== 'function' && typeof v !== 'symbol') {
        acc[key] = canonicalizeObject(v);
      }
      return acc;
    }, {});
}

export function canonicalJson(value: unknown): string {
  return JSON.stringify(canonicalizeObject(value));
}

// ─── Pure TypeScript SHA-256 ──────────────────────────────────────────────────
// Implements SHA-256 without Node.js crypto or browser globals for portability.

function rightRotate(value: number, amount: number): number {
  return (value >>> amount) | (value << (32 - amount));
}

const SHA256_K = [
  0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5,
  0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
  0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3,
  0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
  0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc,
  0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
  0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7,
  0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
  0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13,
  0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
  0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3,
  0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
  0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5,
  0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
  0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208,
  0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2,
];

export function sha256Hash(input: string): DiffHash {
  // UTF-8 encode
  const msgBytes: number[] = [];
  for (let i = 0; i < input.length; i++) {
    let code = input.charCodeAt(i);
    if (code < 0x80) {
      msgBytes.push(code);
    } else if (code < 0x800) {
      msgBytes.push(0xc0 | (code >> 6), 0x80 | (code & 0x3f));
    } else if (code >= 0xd800 && code <= 0xdbff) {
      const hi = code;
      const lo = input.charCodeAt(++i);
      code = ((hi - 0xd800) << 10) + (lo - 0xdc00) + 0x10000;
      msgBytes.push(
        0xf0 | (code >> 18),
        0x80 | ((code >> 12) & 0x3f),
        0x80 | ((code >> 6) & 0x3f),
        0x80 | (code & 0x3f),
      );
    } else {
      msgBytes.push(0xe0 | (code >> 12), 0x80 | ((code >> 6) & 0x3f), 0x80 | (code & 0x3f));
    }
  }

  const bitLen = msgBytes.length * 8;
  msgBytes.push(0x80);
  while (msgBytes.length % 64 !== 56) msgBytes.push(0);
  // Append 64-bit big-endian length
  for (let i = 7; i >= 0; i--) {
    msgBytes.push((bitLen / Math.pow(2, i * 8)) & 0xff);
  }

  let h0 = 0x6a09e667, h1 = 0xbb67ae85, h2 = 0x3c6ef372, h3 = 0xa54ff53a;
  let h4 = 0x510e527f, h5 = 0x9b05688c, h6 = 0x1f83d9ab, h7 = 0x5be0cd19;

  for (let i = 0; i < msgBytes.length; i += 64) {
    const w: number[] = [];
    for (let j = 0; j < 16; j++) {
      w[j] =
        (msgBytes[i + j * 4] << 24) |
        (msgBytes[i + j * 4 + 1] << 16) |
        (msgBytes[i + j * 4 + 2] << 8) |
        msgBytes[i + j * 4 + 3];
    }
    for (let j = 16; j < 64; j++) {
      const s0 = rightRotate(w[j - 15], 7) ^ rightRotate(w[j - 15], 18) ^ (w[j - 15] >>> 3);
      const s1 = rightRotate(w[j - 2], 17) ^ rightRotate(w[j - 2], 19) ^ (w[j - 2] >>> 10);
      w[j] = (w[j - 16] + s0 + w[j - 7] + s1) | 0;
    }

    let [a, b, c, d, e, f, g, h] = [h0, h1, h2, h3, h4, h5, h6, h7];

    for (let j = 0; j < 64; j++) {
      const S1 = rightRotate(e, 6) ^ rightRotate(e, 11) ^ rightRotate(e, 25);
      const ch = (e & f) ^ (~e & g);
      const temp1 = (h + S1 + ch + SHA256_K[j] + w[j]) | 0;
      const S0 = rightRotate(a, 2) ^ rightRotate(a, 13) ^ rightRotate(a, 22);
      const maj = (a & b) ^ (a & c) ^ (b & c);
      const temp2 = (S0 + maj) | 0;
      h = g; g = f; f = e; e = (d + temp1) | 0;
      d = c; c = b; b = a; a = (temp1 + temp2) | 0;
    }

    h0 = (h0 + a) | 0; h1 = (h1 + b) | 0; h2 = (h2 + c) | 0; h3 = (h3 + d) | 0;
    h4 = (h4 + e) | 0; h5 = (h5 + f) | 0; h6 = (h6 + g) | 0; h7 = (h7 + h) | 0;
  }

  return asDiffHash(
    [h0, h1, h2, h3, h4, h5, h6, h7]
      .map((v) => (v >>> 0).toString(16).padStart(8, '0'))
      .join(''),
  );
}

export function hashWeights(weights: ModelWeights | Partial<ModelWeights>): DiffHash {
  return sha256Hash(canonicalJson(weights));
}

// ─── Diff Calculator ──────────────────────────────────────────────────────────

export interface ComputeDiffInput {
  tenantId: TenantId;
  previousVersion: ConfigVersion;
  previousWeights: ModelWeights;
  proposedWeights: Partial<ModelWeights>;
  proposedVersion?: ConfigVersion;
  now?: Date;
}

export function computeModelConfigDiff(input: ComputeDiffInput): ModelConfigDiff {
  const { tenantId, previousVersion, previousWeights, proposedWeights } = input;
  const now = input.now ?? new Date();

  const changedFields = (Object.keys(proposedWeights) as (keyof ModelWeights)[]).filter(
    (k) => proposedWeights[k] !== undefined && proposedWeights[k] !== previousWeights[k],
  );

  const mergedWeights: ModelWeights = { ...previousWeights, ...proposedWeights };

  const configBeforeHash = hashWeights(previousWeights);
  const configAfterHash = hashWeights(mergedWeights);

  const diffPayload = { previousWeights, proposedWeights, changedFields };
  const diffHash = sha256Hash(canonicalJson(diffPayload));

  const proposedVersion = input.proposedVersion ?? asConfigVersion(`${previousVersion}-proposed`);

  return {
    tenantId,
    previousVersion,
    proposedVersion,
    previousWeights,
    proposedWeights,
    changedFields,
    configBeforeHash,
    configAfterHash,
    diffHash,
    computedAt: now,
  };
}
